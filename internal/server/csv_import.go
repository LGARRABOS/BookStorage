...
func (a *App) HandleToolsCSVImport(w http.ResponseWriter, r *http.Request) {
	...
	if err := r.ParseMultipartForm(8 << 20); err != nil {
		http.Redirect(w, r, pathToolsMangaCSV+"?error=form", http.StatusFound)
		return
	}
	...
	if err != nil {
		http.Redirect(w, r, pathToolsMangaCSV+"?error=file", http.StatusFound)
		return
	}
	defer func() { _ = file.Close() }()
	raw, err := io.ReadAll(io.LimitReader(file, csvImportMaxBytes+1))
	if err != nil || len(raw) > csvImportMaxBytes {
		http.Redirect(w, r, pathToolsMangaCSV+"?error=size", http.StatusFound)
		return
	}
	...
	if _, err := a.DB.Exec(
		...
		http.Redirect(w, r, pathToolsMangaCSV+"?error=db", http.StatusFound)
		return
	}
	...
	if err != nil || len(allRows) == 0 {
		...
		http.Redirect(w, r, pathToolsMangaCSV+"?error=parse", http.StatusFound)
		return
	}
	if len(allRows) > csvImportMaxRows {
		_, _ = a.DB.Exec(`DELETE FROM csv_import_sessions WHERE id = ?`, sid)
		http.Redirect(w, r, pathToolsMangaCSV+"?error=rows", http.StatusFound)
		return
	}
	headers := allRows[0]
	preview := allRows
...
func (a *App) handleCSVImportConfirm(w http.ResponseWriter, r *http.Request, userID int) {
	...
	if sid == "" {
		http.Redirect(w, r, pathToolsMangaCSV+"?error=session", http.StatusFound)
		return
	}
	var raw string
	err := a.DB.QueryRow(
	...
	if firstErr != "" && imported == 0 {
		http.Redirect(w, r, pathToolsMangaCSV+"?error="+firstErr, http.StatusFound)
		return
	}
	http.Redirect(w, r, pathToolsManga+"?csv_imported="+strconv.Itoa(imported), http.StatusFound)
}
