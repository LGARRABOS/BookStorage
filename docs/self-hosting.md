...
**Non-interactive release:** set `BSCTL_UPDATE_TAG=v4.2.0` and run `sudo -E bsctl update` to skip the menu. The clone is forced to match the chosen tag or `origin/<branch>` (local changes to tracked files are discarded).
